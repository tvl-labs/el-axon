...
use crate::contract::{
    ...
    service_manager_base::service_manager_base,
...
impl AvsRegistryChainWriter {
    pub async fn new_avs_registry_chain_writer(
        service_manager_addr: Address,
        registry_coordinator_addr: Address,
        operator_state_retriever_addr: Address,
        stake_registry_addr: Address,
    ...
    pub async fn register_operator_in_quorum_with_avs_registry_coordinator(
        ...
        // tracing info
        tracing::info!(avs_service_manager = %self.service_manager_addr, operator= %wallet.address(),quorum_numbers = ?quorum_numbers,"quorum_numbers,registering operator with the AVS's registry coordinator");
    ...
    pub async fn update_stakes_of_entire_operator_set_for_quorums(
        ...
        tracing::info!(quorum_numbers = %quorum_number, "updating stakes for entire operator set");
        let provider = Arc::new(&self.client);
        let wallet = self.tx_mgr.wallet.signer.clone();
        let signer = SignerMiddleware::new(provider.clone(), wallet);
        let contract_registry_coordinator = registry_coordinator::RegistryCoordinator::new(
    ...
    pub async fn update_stakes_of_operator_subset_for_all_quorums(
        ...
        tracing::info!(operators = ?operators, "updating stakes of operator subset for all quorums");
    ...
    pub async fn deregister_operator(
        ...
        tracing::info!("deregistering operator with the AVS's registry coordinator");
        let provider = Arc::new(&self.client);
        let wallet = self.tx_mgr.wallet.signer.clone();
        let signer = SignerMiddleware::new(provider.clone(), wallet);
        let contract_registry_coordinator = registry_coordinator::RegistryCoordinator::new(
