...
use protocol::types::{MemoryAccount, MemoryBackend};
...
fn mock_data(direction: u8, address: evm_types::H160) -> Vec<u8> {
...
fn test_issue_token() {
    ...
    assert_eq!(account.balance, evm_types::U256::from(1000u64));
    assert_eq!(account.nonce, evm_types::U256::from(1u64));
}
...
fn test_burn_token() {
    ...
    state.insert(addr, MemoryAccount {
        ...
        balance: evm_types::U256::from(2000u64),
    ...
    let account = backend.state().get(&addr).unwrap();
    assert_eq!(account.balance, evm_types::U256::from(1000u64));
    assert_eq!(account.nonce, evm_types::U256::from(2u64));
}
...
fn test_burn_token_failed() {
    ...
    state.insert(addr, MemoryAccount {
        nonce:   evm_types::U256::one(),
        balance: evm_types::U256::from(200u64),
        storage: BTreeMap::new(),
        code:    Vec::new(),
    });
    let vicinity = gen_vicinity();
    ...
    let account = backend.state().get(&addr).unwrap();
    println!("{:?}", account);
    assert_eq!(account.balance, evm_types::U256::from(200u64));
    assert_eq!(account.nonce, evm_types::U256::from(1u64));
}
