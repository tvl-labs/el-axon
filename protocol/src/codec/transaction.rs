...
use crate::types::{
    public_to_address, Bytes, Eip1559Transaction, Eip2930Transaction, LegacyTransaction, Public,
    SignatureComponents, SignedTransaction, UnsignedTransaction, UnverifiedTransaction, H256, U256,
};
...
impl Encodable for SignatureComponents {
    fn encode(&self, out: &mut dyn BufMut) {
        if self.is_eth_sig() {
            let r = U256::from_be_slice(truncate_slice(&self.r, 32));
            let s_ = U256::from_be_slice(truncate_slice(&self.s, 32));
            let enc: [&dyn Encodable; 3] = [&self.standard_v, &r, &s_];
            encode_list::<_, dyn Encodable>(&enc, out);
        } else {
            let enc: [&dyn Encodable; 3] = [&self.standard_v, &self.r, &self.s];
            encode_list::<_, dyn Encodable>(&enc, out);
        }
    }
}
...
impl SignatureComponents {
    fn rlp_decode(rlp: &mut &[u8], legacy_v: Option<u64>) -> Result<Self, alloy_rlp::Error> {
        let v: u8 = if let Some(n) = legacy_v {
            SignatureComponents::extract_standard_v(n)
                .ok_or(alloy_rlp::Error::Custom("invalid legacy v in signature"))?
        } else {
            Decodable::decode(rlp)?
        };

        let raw_r = Header::decode_bytes(rlp, false)?;
        let raw_s = Header::decode_bytes(rlp, false)?;
        let eth_tx_flag = raw_r.len() + raw_s.len() <= 64;

        let (r, s) = match eth_tx_flag {
            true => {
                let tmp_r = U256::try_from_be_slice(raw_r).ok_or(alloy_rlp::Error::Overflow)?;
                let tmp_s = U256::try_from_be_slice(raw_s).ok_or(alloy_rlp::Error::Overflow)?;
                (
                    Bytes::from(H256::from(tmp_r).as_slice().to_vec()),
                    Bytes::from(H256::from(tmp_s).as_slice().to_vec()),
                )
            }
            false => (raw_r.to_vec().into(), raw_s.to_vec().into()),
        };
...
pub fn rlp_encode_legacy_tx(
    tx: &LegacyTransaction,
    signature: Option<&SignatureComponents>,
    out: &mut dyn BufMut,
) {
    if signature.is_some() || tx.chain_id.is_some() {
        if let Some(sig) = signature {
            let enc: [&dyn Encodable; 9] = [
                ...
                &tx.value,
                &tx.input,
                &sig.add_chain_replay_protection(tx.chain_id),
                &U256::from_be_slice(truncate_slice(&sig.r, 32)),
                &U256::from_be_slice(truncate_slice(&sig.s, 32)),
            ];
            encode_list::<_, dyn Encodable>(&enc, out);
        } else if let Some(id) = tx.chain_id {
            let enc: [&dyn Encodable; 9] = [
                &tx.nonce,
                &tx.gas_price,
                &tx.gas_limit,
...
pub fn rlp_encode_eip2930_tx(
    tx: &Eip2930Transaction,
    signature: Option<&SignatureComponents>,
    out: &mut dyn BufMut,
) {
    let base_enc: [&dyn Encodable; 8] = [
        &tx.chain_id,
        &tx.nonce,
        &tx.gas_price,
        &tx.gas_limit,
    ...
    if let Some(sig) = signature {
        if sig.is_eth_sig() {
            let r = U256::from_be_slice(truncate_slice(&sig.r, 32));
            let s_ = U256::from_be_slice(truncate_slice(&sig.s, 32));
        ...
        encode_list::<_, dyn Encodable>(&base_enc, out);
    }
}

pub fn rlp_encode_eip1559_tx(
    tx: &Eip1559Transaction,
    signature: Option<&SignatureComponents>,
    out: &mut dyn BufMut,
) {
    let base_enc: [&dyn Encodable; 9] = [
        &tx.chain_id,
        &tx.nonce,
...
pub fn rlp_decode_legacy_tx(buf: &mut &[u8]) -> Result<UnverifiedTransaction, alloy_rlp::Error> {
    ...
    let sig = SignatureComponents::rlp_decode(buf, Some(v))?;
    tx.chain_id = SignatureComponents::extract_chain_id(v);
...
pub fn rlp_decode_eip2930_tx(buf: &mut &[u8]) -> Result<UnverifiedTransaction, alloy_rlp::Error> {
    let mut payload = Header::decode_bytes(buf, true)?;
    ...
    let sig = SignatureComponents::rlp_decode(buf, None)?;
...
pub fn rlp_decode_eip1559_tx(buf: &mut &[u8]) -> Result<UnverifiedTransaction, alloy_rlp::Error> {
    let mut payload = Header::decode_bytes(buf, true)?;
    let id = u64::decode(&mut payload)?;
...
impl Encodable for UnverifiedTransaction {
    fn encode(&self, out: &mut dyn BufMut) {
        if !self.unsigned.is_legacy() {
            out.put_u8(self.unsigned.as_u8());
        }

        match &self.unsigned {
            UnsignedTransaction::Legacy(tx) => {
                rlp_encode_legacy_tx(tx, self.signature.as_ref(), out)
            }
            UnsignedTransaction::Eip2930(tx) => {
                rlp_encode_eip2930_tx(tx, self.signature.as_ref(), out)
            }
            UnsignedTransaction::Eip1559(tx) => {
                rlp_encode_eip1559_tx(tx, self.signature.as_ref(), out)
            }
        }
    }
}
...
impl Decodable for UnverifiedTransaction {
    fn decode(buf: &mut &[u8]) -> Result<Self, alloy_rlp::Error> {
        let header = buf[0];
        if (header & 0x80) != 0x00 {
            return rlp_decode_legacy_tx(buf);
        }

        match header {
            0x01 => rlp_decode_eip2930_tx(&mut &buf[1..]),
            0x02 => rlp_decode_eip1559_tx(&mut &buf[1..]),
...
impl Decodable for SignedTransaction {
    fn decode(buf: &mut &[u8]) -> Result<Self, alloy_rlp::Error> {
        let utx = UnverifiedTransaction::decode(buf)?;
        let sig = utx
            .signature
            .as_ref()
            .ok_or(alloy_rlp::Error::Custom("Missing Signature"))?;

        let (public, sender_addr) = if sig.is_eth_sig() {
            let public = Public::from_slice(
                &secp256k1_recover(utx.signature_hash(true).as_slice(), sig.as_bytes().as_ref())
                    .map_err(|_| alloy_rlp::Error::Custom("recover signature"))?
            ...
            (
                ...
                sig.extract_interoperation_tx_sender()
                    .map_err(|_| alloy_rlp::Error::Custom("Invalid interoperation sender"))?,
            )
        };
...
mod tests {
    ...
    use crate::types::{Address, Public, SignatureS, Witness, U256, U64};
    ...
    fn test_legacy_decode() {
        ...
        let tx = UnverifiedTransaction::decode(&mut bytes.as_ref()).unwrap();

        assert!(tx.check_hash().is_ok());
        assert!(tx.unsigned.data().is_empty());
        assert_eq!(*tx.unsigned.gas_limit(), 0x5208u64);
        assert_eq!(tx.unsigned.gas_price(), 0x01u128);
        assert_eq!(*tx.unsigned.nonce(), 0x00u64);
        assert_eq!(
            tx.unsigned.to().unwrap(),
            Address::from_slice(&hex_decode("095e7baea6a6c7c4c2dfeb977efac326af552d87").unwrap())
        );
        assert_eq!(*tx.unsigned.value(), U256::from(0x0au64));
        assert_eq!(
            public_to_address(&tx.recover_public(false).unwrap()),
            Address::from_slice(&hex_decode("0f65fe9276bc9a24ae7083ae28e2660ef72df99e").unwrap())
        );
        assert_eq!(tx.unsigned.chain_id(), Some(0u64));
    ...
    fn test_legacy_decode_without_chain_id() {
        ...
        assert_eq!(tx.unsigned.chain_id(), None);
    }
    ...
    fn test_legacy_decode_with_chain_id() {
        ...
        assert!(tx.unsigned.data().is_empty());
        assert_eq!(*tx.unsigned.gas_limit(), 0x5208u64);
        assert_eq!(tx.unsigned.gas_price(), 0x02540be400u128);
        assert_eq!(*tx.unsigned.nonce(), 0x05u64);
        assert_eq!(
            tx.unsigned.to().unwrap(),
            Address::from_slice(&hex_decode("f386573563c3a75dbbd269fce9782620826ddac2").unwrap())
        );
        assert_eq!(*tx.unsigned.value(), U256::from(0x0de0b6b3a7640000u64));
        assert_eq!(
            public_to_address(&tx.recover_public(false).unwrap()),
            Address::from_slice(&hex_decode("6fff69c55448d20501b848ebbde255f9a6b98ba0").unwrap())
        );
        assert_eq!(tx.unsigned.chain_id(), Some(2022u64));
    }
    ...
    fn test_legacy_decode_eip_1820() {
        ...
        assert_eq!(*tx.unsigned.gas_limit(), 0x0c3500u64);
        assert_eq!(tx.unsigned.gas_price(), 0x174876e800u128);
        assert_eq!(*tx.unsigned.nonce(), 0x00u64);
        assert_eq!(
            tx.unsigned.data(),
        ...
        assert_eq!(
            public_to_address(&tx.recover_public(false).unwrap()),
            Address::from_slice(&hex_decode("a990077c3205cbdf861e17fa532eeb069ce9ff96").unwrap())
        );
        assert_eq!(tx.unsigned.chain_id(), None);
    }
    ...
    fn test_signed_tx_codec() {
        ...
        let utx = UnverifiedTransaction::decode(&mut raw.as_ref()).unwrap();
        let origin = SignedTransaction::from_unverified(utx).unwrap();

        let mut encode = BytesMut::new();
        origin.encode(&mut encode);
        let decode = SignedTransaction::decode(&mut encode.as_ref()).unwrap();
    ...
    fn test_decode_unsigned_tx() {
        ...
        let res = UnverifiedTransaction::decode(&mut raw.as_ref()).unwrap();
        assert!(res.check_hash().is_ok());

        let bytes = hex_decode("f86308018252089423c812dcf2b48cd5dcc7d354b1892fec7047f9348203e880820ff0a0753e6fee49a95f6a9ab6411c4d924e8c4260ef16857a26b867b1995d2bcab401a02bfcca1c0cb2b456c4d7de081fac0e1730bae46d45adda3d77bb2bbbe54a4f29").unwrap();
        let res = UnverifiedTransaction::decode(&mut bytes.as_ref()).unwrap();
        assert!(res.check_hash().is_ok());
    }
    ...
    fn test_legacy_encode() {
        ...
        let mut encode = BytesMut::new();
        utx.encode(&mut encode);
        let recover = UnverifiedTransaction::decode(&mut encode.as_ref()).unwrap();
    ...
    fn test_legacy_encode_eip_1820() {
        let raw = hex_decode("").unwrap();
        let tx = UnverifiedTransaction::decode(&mut raw.as_ref()).unwrap();

        let mut encode = BytesMut::new();
        tx.encode(&mut encode);
        let recover = UnverifiedTransaction::decode(&mut encode.as_ref()).unwrap();

        assert_eq!(tx, recover);
    }
    ...
    fn should_agree_with_vitalik() {
        let test_vector = |tx_data: &str, address: &'static str| {
            let raw = hex_decode(tx_data).unwrap();
            let utx = UnverifiedTransaction::decode(&mut raw.as_ref()).unwrap();
            let signed = SignedTransaction::from_unverified(utx.clone()).unwrap();
            assert_eq!(
                signed.sender,
                Address::from_slice(&hex_decode(address).unwrap())
            );
            assert!(utx.check_hash().is_ok());
            assert!(utx.check_hash().is_ok());
    ...
    fn test_interoperation_tx_codec() {
        ...
        let utx = UnverifiedTransaction::decode(&mut raw_tx.as_ref())
            .unwrap()
            .calc_hash();
        let sig = utx.signature.unwrap();
        ...
        assert_eq!(
            ...
            Address::from_slice(&hex_decode("Cb9112D826471E7DEB7Bc895b1771e5d676a14AF").unwrap())
        );
        assert!(utx.unsigned.data().is_empty());
        assert!(*utx.unsigned.nonce() == 0);
        assert!(*utx.unsigned.gas_limit() == 0);
        assert!(utx.unsigned.gas_price() == 0);
        assert!(*utx.unsigned.max_priority_fee_per_gas() == 0);
        assert!(utx.unsigned.chain_id() == Some(5));
        assert_eq!(sig.standard_v, 0);
        assert_eq!(sig.r, hex_decode("").unwrap());
    }
    ...
    fn test_signature_s_codec() {
        ...
        assert_eq!(SignatureS::decode(&mut encoded.as_ref()).unwrap(), s);
    }
}
