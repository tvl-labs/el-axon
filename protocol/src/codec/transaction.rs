...
pub fn rlp_decode_legacy_tx(buf: &mut &[u8]) -> Result<UnverifiedTransaction, alloy_rlp::Error> {
    ...
    let sig = SignatureComponents::rlp_decode(&mut payload, Some(v))?;
...
pub fn rlp_decode_eip2930_tx(buf: &mut &[u8]) -> Result<UnverifiedTransaction, alloy_rlp::Error> {
    ...
    let sig = SignatureComponents::rlp_decode(&mut payload, None)?;
    ...
    fn encode(&self, out: &mut dyn BufMut) {
        ...
        match &self.unsigned {
            UnsignedTransaction::Legacy(tx) => {
                rlp_encode_legacy_tx(tx, self.signature.as_ref(), out, true)
            }
...
mod tests {
    ...
    use crate::types::{Address, Public, SignatureS, Witness, U256};
